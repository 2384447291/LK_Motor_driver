...
class USB2CAN(object):  
    ...
    def CanSend(self,CAN_ID,Data):
        ...
        CAN_ID=list(struct.pack("I",CAN_ID))
        send_data[13:17] = CAN_ID 
...
class CanMsgCenter():
    def __init__(self):
        ...
        self.registedmotor = []
        self.num_motor = 0
        self.LKmotorTorqueBuffer = np.zeros((8,1),np.uint8) 
        self.need_send_torque_message = False

    def register(self,Motor):
        self.Motordecitionary[Motor.id+0x140] = Motor
    ...
    def UpdateMassage(self,_interface):
        ...
        #     canid = 0x280
        #     tempmsg = canMsg(canid,self.LKmotorTorqueBuffer.reshape(8,))
        #     self.SendMsgQue.put(tempmsg) 
        #     self.need_send_torque_message = False
        #真正的处理发送
        packettosend = 4
        ###########################在这里控制电机
            ...
            if self.Recieve_step == Step["checking_head"]:
                ...
                if self.ThispacketLength == 16: 
                    if _data == 0x55:
                        ...
                        elif self.Thispacket[1] == 0x11:#接收成功
